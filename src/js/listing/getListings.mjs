...
export function getListingsTemplet(listings) {
  ...
  if (listings) {
    listings.map(
      (listing) =>
        (listingsDiv.innerHTML += `<a href="/auction-house/view-detail/index.html?id=${
          listing.id
        }"  class="p-2 p-xl-3 col-sm-6 col-md-4 col-lg-3 loader-div listing-card mt-5 shadow new">
          <div class="container border-0">
             <img  id="img" class="img-thumbnail listing-image  rounded" src="${
               listing.media[0]
             }" onerror="src='/assets/images/image-default.jpg'"   />
            <div class="text-center">
             <h4 class="text-capitalize flex-shrink-1 my-1">${
               listing.title
             }</h4>
             <div fw-bold>Time Left: <span text-primary f-large>${countDown(
               listing.endsAt
             )}</span></div>
