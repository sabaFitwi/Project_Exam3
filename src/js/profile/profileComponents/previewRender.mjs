export const imageCarousel = function (media) {
  ...
  media.forEach((image, index) => {
    bigImage.innerHTML += `<div class="carousel-item${
      index === 0 ? " active" : ""
    }">
                                        <div >
                                          <div>
                                            <img id="listing-image" src="${image}" onerror="src='/assets/images/image-defualt.jpg'" alt="Item image ${
      index + 1
    }" />
                                          </div>
                                        </div>
                                      </div>`;
    smallImage.innerHTML += `<a type="button" data-bs-target="#specific" data-bs-slide-to="${index}" class="thumbs ${
      index === 0 ? " active" : ""
